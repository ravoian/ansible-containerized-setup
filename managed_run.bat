...
:: -----------------------------------------------------------------
set NAME="ansible-agent-node"


:: -----------------------------------------------------------------
:: --- Setup docker network for static ip allocation
:: -----------------------------------------------------------------
set NETWORK=temp-net
...
:: -----------------------------------------------------------------
echo Building %NAME%
docker build -t %NAME% -f managed.Dockerfile .
...
docker run -d --rm --net %NETWORK% --ip 172.18.0.22 --name %NAME% "%NAME%"
